...
def runOnCommandline(firstScript: String, otherScripts: String*)(implicit dir: File): Int = {
  def command(script: String) = {
    if(isWindows){ Process("cmd /c set CI=true&&" + script, dir) } else { Process("env CI=true " + script, dir) }
  }
  otherScripts
    .foldLeft(command(firstScript)) {
      (acc, other) => acc #&& command(other)
    }
} !
...
def executeUiCodeStyle(implicit dir: File): Int = ifNodeModulesInstalled(runOnCommandline(FrontendCommands.prettier, FrontendCommands.eslint))

def executeUiCodeStyleCheck(implicit dir: File): Int = ifNodeModulesInstalled(runOnCommandline(FrontendCommands.prettierCheck, FrontendCommands.eslintCheck))
